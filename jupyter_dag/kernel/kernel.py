...
Where this sits in the message flow
-----------------------------------
...
1. ``analyze_request`` (shell and control channels): "which names does each of these cells define
   and reference?" Answered by :func:`jupyter_dag.analysis.analyze_cells` and sent back as
   ``analyze_reply``.
2. ``execute_request`` gains two optional fields, ``namespace_delete`` (unbind these names before
   running) and ``namespace_set`` (bind these JSON values first), handled in :meth:`DagKernel.do_execute`.
...
(:attr:`DagKernel.kernel_info`), so a frontend can feature-detect before sending anything new.

Ways to get this kernel
-----------------------
A kernelspec launches the stock ``ipykernel_launcher`` with
...
such spec and :mod:`jupyter_dag.kernel.install` writes one for any interpreter. The other way is
``%load_ext jupyter_dag`` inside an already running stock kernel: :func:`load_ipython_extension` at
the bottom of this file swaps the live kernel's class. That path exists so the prototype can be
tried without installing a kernelspec.

The same payloads are also reachable over a comm target (:mod:`jupyter_dag.kernel.comm`), which
is the no-protocol-change transport: it works on any frontend that can open a comm, at the price
of not being a real message type.
"""
...
class DagKernel(IPythonKernel):
    ...
    How a new message type gets a handler
    -------------------------------------
    ``Kernel.__init__`` (``ipykernel/kernelbase.py:312-317``) builds the two dispatch tables from
    class-level lists::
    ...
    The tables are plain dicts and are never rebuilt, so :meth:`_init_dag` adds ``analyze_request``
    to both of them directly, after construction. That is also how ipykernel itself adds the comm
    handlers (``ipykernel/ipkernel.py:163-165``), and it is the one mechanism that works both for a
    kernel constructed as this class and for a running kernel that :func:`load_ipython_extension`
    ...
    grafted onto a running kernel by :func:`load_ipython_extension`. In both cases
    :meth:`_init_dag` performs the per-instance setup.
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        """Construct the kernel as ipykernel does, then add the handler and the comm target.
    ...
    def _init_dag(self) -> None:
        ...
        Split out of :meth:`__init__` because :func:`load_ipython_extension` swaps the class of a
        kernel that has already been constructed, so the ``__init__`` of this class never runs on
        that path; it calls this method instead.
        ...
        me": see :class:`jupyter_dag.kernel.comm.DagCommTarget`.
        """
    ...
    def kernel_info(self) -> dict[str, Any]:
        ...
        strings follow ipykernel's spelling convention: lower case, space separated
        (:data:`jupyter_dag.protocol.ALL_FEATURES`).
    ...
    async def analyze_request(self, stream: Any, ident: Any, parent: dict[str, Any]) -> None:
        ...
        by message type and await it (``kernelbase.py:476`` and ``:375``). ``stream`` is the ZMQ
        stream the request arrived on, ``ident`` the ZMQ routing identities of the sender, and
        ``parent`` the full request message. All three go back into ``session.send``
        (``jupyter_client/session.py:760``): the reply's ``parent_header`` is copied from ``parent``,
        which is how the frontend matches the reply to its pending request, and ``ident`` routes
        it to the right connected client. ``is_complete_request`` (``kernelbase.py:1113``) is the
        base-class handler this one is modelled on.
        ...
        shell state: :mod:`jupyter_dag.analysis` parses the source with its own stateless
        ...
        parent : dict
            ...
            ``{"cell_id", "code"}`` dicts (:class:`jupyter_dag.protocol.AnalyzeCellInput`).
        """
        if not self.session:
            return
        try:
    ...
    async def do_execute(self, **kwargs: Any) -> dict[str, Any]:
        ...
        Why the signature is ``**kwargs``
        ---------------------------------
        ``execute_request`` (``kernelbase.py:786``) calls ``do_execute`` with keyword arguments only
        (``kernelbase.py:831``): ``code``, ``silent``, ``store_history``, ``user_expressions`` and
        ``allow_stdin``, plus ``cell_meta`` and ``cell_id`` when the signature accepts them
        (``kernelbase.py:76-88`` treats ``**kwargs`` as accepting both). Forwarding them untouched
        to ``IPythonKernel.do_execute`` (``ipykernel/ipkernel.py:374``) means this method does not
        have to track ipykernel's parameter list.

        Why the extra request fields are read from the parent message
        -----------------------------------------------------------------
        ``execute_request`` unpacks a fixed set of fields from the request content and ignores the
        rest, so ``namespace_delete`` and ``namespace_set`` are fetched from ``self.get_parent()``
        (``kernelbase.py:694``), which returns the message currently being handled on this channel.

        Why the reply can simply carry ``namespace_delta``
        -------------------------------------------------
        The dict this method returns is sent verbatim as the ``execute_reply`` content
        (``kernelbase.py:859``). Adding a key adds a field to the protocol message; clients that do
        not know it ignore it, and every connected client gets it, whichever of them sent the
        request. The frontend reads it off every ``execute_reply`` it sees, so there is no second
        delivery path.

        Parameters
        ----------
        **kwargs
            As passed by ``execute_request``; see above.
        ...
        and gets back ``{"status": "ok", ..., "namespace_delta": {"added": [], "removed": ["df", "model"]}}``.
        """
...
def load_ipython_extension(ipy: InteractiveShell) -> None:
    ...
    (``IPython/core/extensions.py:53``). ``jupyter_dag/__init__.py`` forwards here lazily so
    that importing the package never imports ipykernel.

    How the swap works
    ------------------
    The shell holds a reference to its kernel (``ZMQInteractiveShell.kernel``,
    ...
    lookup finds the DagKernel overrides first (``do_execute``, ``kernel_info``,
    ``analyze_request``), and the base class's ``__init__``-time state (session, sockets, comm
    manager) is untouched because no ``__init__`` runs. What ``__init__`` would have done is
    repeated by calling :meth:`DagKernel._init_dag`, which fills the handler tables and registers
    the comm target.

    The generated class also records the class it replaced, as ``_dag_prev_class``, so
    :func:`unload_ipython_extension` can put it back. Storing it on the class rather than in a
    module global keeps the fact with the swap: each ``%load_ext`` makes a fresh class with its own
    pointer, and once the swap is undone the attribute stops resolving by itself.

    Limitation
    ----------
    ``kernel_info_reply`` was answered when the frontend connected, before this ran, so the
    frontend's cached ``supported_features`` do not include the jupyter-dag strings. The frontend
    (``src/dag/protocol.ts``) therefore falls back to the comm transport, whose ``features``
...
def unload_ipython_extension(ipy: InteractiveShell) -> None:
    """Undo :func:`load_ipython_extension`; the target of ``%unload_ext jupyter_dag``.

    Restores the class recorded at load time and removes the ``analyze_request`` entries from both
    handler tables. The comm target stays registered on the shared ``CommManager``
    (``unregister_target`` exists, ``comm/base_comm.py:229``, but :class:`DagCommTarget` keeps no
    handle for it yet); comms already open keep their handlers regardless.
